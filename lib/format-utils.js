...
exports.chooseFormat = (formats, options) => {
  ...
  switch (quality) {
    case "highest":
      ...
      break;
      case "360p":
        selectedFormat = formats.filter(format => {
          return format.qualityLabel === "360p" && format.mimeType.split(";")[0] === "video/mp4"
        })
        format = selectedFormat[0];
        break;
        case "720p":
        selectedFormat = formats.filter(format => {
          return format.qualityLabel === "720p" && format.mimeType.split(";")[0] === "video/mp4"
        })
        format = selectedFormat[0];
        break;
        case "1080p":
        selectedFormat = formats.filter(format => {
          return format.qualityLabel === "1080p" && format.mimeType.split(";")[0] === "video/mp4"
        })
        format = selectedFormat[0];
        break;
      case "144p":
        selectedFormat = formats.filter(format => {
          return format.qualityLabel === "144p" && format.mimeType.split(";")[0] === "video/mp4"
        })
        format = selectedFormat[0];
        break;
      case "480p":
        selectedFormat = formats.filter(format => {
          return format.qualityLabel === "480p" && format.mimeType.split(";")[0] === "video/mp4"
        })
        format = selectedFormat[0];
