...
const timestampPattern = /(?:signatureTimestamp|sts):(\d+)/;
const nFunctionPattern = "function\\(\\s*(\\w+)\\s*\\)\\s*\\{" +
...
const extractDecipher = body => {
  ...
  if (!decipherFunc && !decipherWarning) {
    console.warn(
      ...
      `https://github.com/ToddyTheNoobDud/ytdl-core-toddys/issues`,
    );
...
const extractNTransformFunc = body => {
  try {
    const nFunc = matchFirst(nFunctionPattern, body);
...
const extractNTransformWithName = body => {
  try {
    ...
    const funcPattern = `(${
      nFuncName.replace(/\$/g, "\\$")}=\\s*function([\\S\\s]*?\\}\\s*return (([\\w$]+?\\.join\\(""\\))|(Array\\.prototype\\.join\\.call\\([\\w$]+?,[\\n\\s]*(("")|(\\("",""\\)))\\)))\\s*\\})`;
