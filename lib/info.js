...
exports.getInfo = async (id, options) => {
  ...
  try {
    ...
    if (options.playerClients.includes("WEB_EMBEDDED"))
      clientPromises.push(fetchWebEmbeddedPlayer(id, info, options));
    if (options.playerClients.includes("TV"))
      clientPromises.push(fetchTvPlayer(id, info, options));
    if (options.playerClients.includes("IOS"))
      clientPromises.push(fetchIosJsonPlayer(id, options));
    ...
    if (clientPromises.length > 0) {
      ...
      if (successfulResponses.length > 0) {
        ...
        const formats = parseFormats(bestPlayerResponse);
        if (formats && formats.length > 0) {
          formatPromises.push(sig.decipherFormats(formats, info.html5player, options));
        }
        
        const manifestPromises = parseAdditionalManifests(bestPlayerResponse, options);
